...
class Discriminator(BaseNetwork):
    def __init__(self, in_channels, use_sigmoid=True, use_spectral_norm=True, init_weights=True):
        ...
        #     spectral_norm(nn.Linear(512, 105), use_spectral_norm),
        # )
    ...
    def forward(self, x):
        ...
        # gap = nn.functional.adaptive_avg_pool2d(conv4, (1,1))
        # fc = self.fc(gap)

        outputs = conv5
        # outputs_ = tensor.softmax(fc, 1)
        if self.use_sigmoid:
            outputs = torch.sigmoid(conv5)
        # print(conv1.shape ,conv2.shape, conv3.shape, conv4.shape, conv5.shape, outputs.shape)
